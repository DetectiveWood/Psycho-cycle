...
class VisualDayPlanningGame:
    def show_intro_sequence(self):
        """Атмосферное вступление от первого лица"""
        intro_music_path = "intro/music.ogg"  # ← ПУТЬ К ТВОЕЙ МУЗЫКЕ (замени на свой файл)

        try:
            pygame.mixer.music.load(intro_music_path)
            pygame.mixer.music.play(-1)  # Зацикливаем музыку (-1 = бесконечно)
            pygame.mixer.music.set_volume(0.6)  # Громкость (0.0–1.0)
        except pygame.error as e:
            print(f"Ошибка загрузки музыки вступления: {e}")

        intro_frames = [
            ("intro/clock.png",
             "Каждый день похож на предыдущий. Порой, отключая будильник, я ощущаю, будто до сих пор \nнахожусь во сне. В страшном, бесконечном и тревожном сне. "),
            ("intro/mirror.png",
             "Я смотрю в зеркало, но не узнаю себя. У меня всегда были эти синяки? И седые волосы?.. \nЯ чувствую, что каждый день мне становится только хуже. "),
            ("intro/window.png",
             "Я чувствую, что они следят за мной. Никому нельзя доверять. Каждый мой шаг отслеживается...\nНужно быть аккуратнее и заметать свои следы ещё лучше. "),
            ("intro/shadow.jpeg",
             "Каждый день - это борьба. Я стараюсь спрятаться, убежать от них. Хотя может и от себя тоже?.. \nВ любом случае окончательного успеха я так и не достиг. "),
            ("intro/phone.jpeg",
             "Они мне постояно звонят. Я не беру трубку, блокирую, но звонки продолжаются. \nКаждый раз мне удается заснуть и все повторяется вновь... \nКак же разорвать этот ужасный цикл?"),
        ]

        # Надёжный шрифт для русского языка
        font = pygame.font.SysFont("arial", 30)  # основной текст, 36 — оптимальный размер

        for image_path, full_text in intro_frames:
            # Фон
            ...
            if image_path:
                try:
                    ...
                    scaled = pygame.transform.smoothscale(img, (new_width, new_height))
            ...
            # Нижняя область для текста (полупрозрачный фон)
            text_area_height = 220
            text_bg = pygame.Surface((WINDOW_WIDTH, text_area_height), pygame.SRCALPHA)
            text_bg.fill((0, 0, 0, 160))  # полупрозрачный чёрный
            self.screen.blit(text_bg, (0, WINDOW_HEIGHT - text_area_height))

            # Эффект печатной машинки
            lines = full_text.split('\n')
            current_y = WINDOW_HEIGHT - text_area_height + 40  # отступ сверху области
            ...
            for line in lines:
                ...
                for char in line:
                    displayed_text += char
                    text_surf = font.render(displayed_text, True, (220, 220, 240))
                    text_rect = text_surf.get_rect(left=x, centery=current_y)
                    self.screen.blit(text_surf, text_rect)
                    pygame.display.flip()  # обновляем экран после каждой буквы
                    pygame.time.wait(60)  # скорость печати — 60 мс на символ (можно менять)

                current_y += 50  # расстояние между строками

            pygame.display.flip()

            # Ожидание: 5 секунд или клик/клавиша
    ...
    def show_end_sequence(self, ending_type):

        info = self.get_ending_and_tasks_info()
        print(f"Концовка: {info['ending_type']}")
        print(f"Задания: {info['tasks_completed']}/{info['tasks_total']} ({info['tasks_percent']}%)")

        intro_frames = [
            ("end/start.png",
             "Я захожу и поднимаюсь в десятиэтажная заброшку на самом краю нашего городка. Прямо рядом \nс лесом. Возможно стоило просто уйти? Вдруг это тоже ловушка... Но я не могу. Ответы важнее \nвсего. Я готов рискнуть..."),
            ("end/happy.png",
             "Слова голоса отдаются эхом. Я как будто слышу их со всех сторон одновременно. Я чувствую \nкак он доволен мной... Я даже начинаю видеть полупрозрачное лицо прямо перед собой. Я \nчувствую странное умиротворение... "),
            ("end/unhappy.jpeg",
             "Слова голоса отдаются эхом. Я как будто слышу их со всех сторон одновременно. Я чувствую \nкак он недоволен мной... Я даже начинаю видеть полупрозрачное лицо прямо перед собой. Я \nчувствую странное волнение... "),
            ("end/restart.png",
             "Внезапно меня одолевает неимоверная усталось. Голос начинает звучать все более тихо и невнятно. \nЯ стараюсь устоять на ногах, но не могу. Что-то неведомое тянет меня вниз. У меня нет сил \nсражаться дальше. Цикл не разорвать... "),
            ("end/fall.png",
             "Голос начинает звучать все более тихо и невнятно. Я не хочу его слушать. Ему нельзя доверять. \nНикому нельзя доверять... Сопротивление более не имеет смысла. Я потерпел поражение \nот самого себя. Я подхожу к краю крыши и делаю шаг вперед... "),
            ("end/true.png",
             "Голос начинает звучать все более тихо и невнятно. Я не хочу его слушать. Он пытался провести \nменя. Однако теперь я готов. Я более не нуждаюсь в его 'указаниях' и 'помощи' Незнакомка \nоткрыла мне глаза, а встреча с котом указала путь. Теперь я знаю что делать... ")
        ]

        # Надёжный шрифт для русского языка
        font = pygame.font.SysFont("arial", 30)  # основной текст, 36 — оптимальный размер

        if info['tasks_percent'] < 50:
            intro_frames.remove(intro_frames[1])
        if info['tasks_percent'] >= 50:
            intro_frames.remove(intro_frames[2])
        if not (info['ending_type'] == "good" or info['ending_type'] == "neutral" or info['ending_type'] == "bad"):
            intro_frames.remove(intro_frames[2])
        if info['ending_type'] != "special_doubt":
            intro_frames.remove(intro_frames[-2])
        if info['ending_type'] != "special_both":
            intro_frames.remove(intro_frames[-1])
        for image_path, full_text in intro_frames:
            # Фон
            self.screen.fill((10, 10, 20))
    ...
    def get_ending_and_tasks_info(self):
        # Тип концовки
        if hasattr(self.game_state, 'current_ending') and self.game_state.current_ending:
            ending_type = self.game_state.current_ending.get('type', 'unknown')
        else:
            ending_type = 'not_reached_yet'

        # Задания
        completed = len(self.game_state.completed_voice_tasks)
        total = len(self.game_state.voice_tasks) + completed
    ...
    def handle_events(self):
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    if self.game_state.phase == 'planning':
                        ...
                        self.new_day()
                    elif self.game_state.phase == 'game_ending' or self.game_state.phase == 'game_over':
        ...
        if mouse_clicked:
            ...
            if (self.buttons['start_day'].update(mouse_pos, True) and
                ...
                self.start_day()
            if self.buttons['start_day'].update(mouse_pos, True) and self.game_state.phase == 'game_over':
                    self.restart_game_completely()
            ...
            # New day button
                ...
                self.new_day()
            elif self.buttons['new_day'].update(mouse_pos, True) and self.can_click_button('new_day'):
                self.register_button_click('new_day')
                if  self.game_state.phase == 'game_ending' or self.game_state.phase == 'game_over':
                    self.restart_game_completely()
    ...
    def continue_execution(self):
        ...
        # Очищаем защиту от штрафа после проверки (чтобы событие внутри активности не считалось простоем)
        if hasattr(self.game_state, 'event_time_skip_active') and self.game_state.event_time_skip_active:
            print("💫 Очищаем флаг защиты после проверки штрафа при возобновлении активности")
            self.game_state.event_time_skip_active = False
            if hasattr(self.game_state, 'event_time_skip_start'):
                del self.game_state.event_time_skip_start
            if hasattr(self.game_state, 'event_time_skip_end'):
                del self.game_state.event_time_skip_end
        ...
        # 🛡️ ПРЕДВАРИТЕЛЬНАЯ ЗАЩИТА: Устанавливаем флаг ДО обработки активности
        # чтобы правильно вычислить стресс свободного времени
        event_occurred_during_activity = False
        if (hasattr(self.game_state, 'show_event_popup') and
                self.game_state.show_event_popup and
                hasattr(self.game_state, 'current_event')):
            event_occurred_during_activity = True
            print(f"🛡️ СОБЫТИЕ ВО ВРЕМЯ АКТИВНОСТИ - БУДЕТ УСТАНОВЛЕНА ЗАЩИТА")

        # Apply activity effects
        self.process_activity(current_activity)

        # 🛡️ ПРИМЕНЯЕМ ЗАЩИТУ: Если событие произошло во время активности
        if event_occurred_during_activity:
        ...
        if self.game_state.current_activity_index < len(scheduled_activities):
            # Есть следующая активность
            next_activity = scheduled_activities[self.game_state.current_activity_index]
            gap_start = self.game_state.current_time
            gap_end = next_activity.start_time

            if gap_end > gap_start + 0.1:  # есть промежуток >6 минут
                free_time_hours = gap_end - gap_start
                free_time_minutes = int(free_time_hours * 60)
                base_penalty = (free_time_minutes // 10) * 4  # 4 стресса за 10 минут
                paranoia_modifier = 1 + (self.game_state.paranoia / 100) * 0.5
                final_penalty = int(base_penalty * paranoia_modifier)

                if final_penalty > 0:
                    self.game_state.stress = min(100, self.game_state.stress + final_penalty)
                    print(f"😟 +{final_penalty} СТРЕССА ОТ ПРОСТОЯ ({free_time_hours:.1f}ч между активностями)")
                    self.game_state.add_message(f"+{final_penalty} стресса от простоя ({free_time_hours:.1f}ч)",
                                                'warning')

                # Продвигаем время до начала следующей активности
                self.game_state.current_time = gap_end

        else:
            # Последняя активность дня закончена — проверяем до конца дня (22:00)
            end_of_day = 22.0  # или 23.0, если у тебя день до 23
            gap_start = self.game_state.current_time

            if end_of_day > gap_start + 0.1:
                free_time_hours = end_of_day - gap_start
                free_time_minutes = int(free_time_hours * 60)
                base_penalty = (free_time_minutes // 10) * 4
                paranoia_modifier = 1 + (self.game_state.paranoia / 100) * 0.5
                final_penalty = int(base_penalty * paranoia_modifier)

                if final_penalty > 0:
                    self.game_state.stress = min(100, self.game_state.stress + final_penalty)
                    print(f"😟 +{final_penalty} СТРЕССА ОТ ПРОСТОЯ ({free_time_hours:.1f}ч в конце дня)")
                    self.game_state.add_message(f"+{final_penalty} стресса от простоя ({free_time_hours:.1f}ч)",
                                                'warning')

                self.game_state.current_time = end_of_day
    ...
    def process_activity(self, activity):
        ...
        if raw_energy_recovery > 0:
            effective_energy_recovery = raw_energy_recovery * (1 - paranoia_factor * 0.7)
        else:
        ...
        if raw_stress_change > 0:  # Positive stress change (bad)
            effective_stress_change = raw_stress_change
        ...
        if random.random() < 0.05:
            ...
            if available_events:
                ...
                if (hasattr(self.game_state, 'current_activity_index') and
                        self.game_state.current_activity_index < len(self.game_state.timeline_activities)):
                    scheduled_activities = self.game_state.get_planned_schedule()
                    if self.game_state.current_activity_index < len(scheduled_activities):
                ...
                # 🛡️ УСТАНАВЛИВАЕМ ЗАЩИТУ ДО ПОКАЗА ПОПАПА (ДО того как обработается стресс)
                max_time_shift = 5.0
                protection_end = min(22.0, current_time_before_event + max_time_shift)
                ...
                # Показываем popup события (ПОСЛЕ установления защиты)
                self.game_state.current_event = event
                self.game_state.show_event_popup = True
    ...
    def use_card(self, card):
        ...
        # Handle status-type cards (new cards with hourly effects)
        if card.get('effect_type') == 'status':
            effect = card.get('effect', {})
            status_id = effect.get('status_id')
            status_name = effect.get('status_name', card['name'])
            status_icon = effect.get('status_icon', card['icon'])
            duration_hours = effect.get('duration_hours', 0)
            ...
            self.game_state.add_message(f"{card['icon']} {card['name']} активирована! ({duration_hours} часов)",
                                        'success')
            return
    ...
    def transition_status_effects_to_new_day(self):
        ...
        end_of_day_time = 23.0
        new_day_start_time = 6.0
    ...
    def new_day(self):
        ...
        if self.game_state.phase == 'game_over':
            ...
            self.game_state.current_time = 6.0
            self.game_state.current_activity_index = 0
    ...
    def trigger_game_ending(self):
        ...
        VisualDayPlanningGame.show_end_sequence(self, ending_data)
        # Show ending message
    ...
    def get_dream_database(self):
        ...
        return {
            ...
            'warning': [
                ...
                {
                    'id': 'warning_store',
                    'name': 'Ловушка потребления',
                    'descriptions': [
                        'Магазин тянет вас внутрь. Полки манят ненужными товарами. Вы тратите без конца...',
                        'В торговом центре вас окружают соблазны. Кошелёк становится всё тоньше. Контроль теряется...',
                        'Вы видите себя в магазине, покупающим всё подряд. Деньги исчезают. Это ошибка...'
                    ],
                    'effects': {'stress': 5, 'paranoia': 3},
                    'hint': 'Опасайтесь излишних трат сегодня...',
                    'avoid_location': 'store',
                    'penalty': {'stress': 12, 'energy': -8}
                }
            ],
    ...
    def complete_dream_phase(self):
        ...
        # ВАЖНО: Сообщение о начале дня будет добавлено в new_day(), а не здесь
        # Чтобы избежать дублирования при смене фазы
    ...
    def determine_ending(self, stress, paranoia):
        ...
        # CHECK FOR SPECIAL ENDINGS BASED ON PERMANENT STATUSES
        has_cat_blessing = (hasattr(self.game_state, 'permanent_statuses') and
                            'cat_blessing' in self.game_state.permanent_statuses)
        has_doubt = (hasattr(self.game_state, 'permanent_statuses') and
                     'doubt' in self.game_state.permanent_statuses)
        ...
        description_4 = (
            "Вдруг вы почувствовали сомнения. Вы доверяли внутреннему голосу все эти 30 дней, но стоило ли оно того? "
            "Его слова... В них точно что-то не так. Что же делать?")
        description_5 = (
            "Он ваш друг? Разве его действия были направлены на помощь? Вы вспоминаете черного кота, которого однажды встретили на улице. "
            "Вы помните как жалобно он смотрел на вас и как вы ему помогли. После этого, вы чувствовали как этот кот вам помогал. "
            "Пускай его не было видно, но внутри вы ощущали, что он всегда рядом. Его можно назвать вашим другом. А что насчет внутреннего голоса? "
            "Он превратил вашу жизнь в испытание. Просили ли вы его об этом? Возможно. Но ведь не все так просто. Вы чувствуете, что вас использовали...")
        description_6 = (
            "Почему-то вы почувствовали себя лучше чем обычно. Вы повстречали замечательного котика и это греет вам душу. "
            "Возможно все то, что рассказал вам голос так же было правдой? Учитывая, что все начинает налаживаться, вы в это верите. ")

        # SPECIAL ENDING: Both cat blessing and doubt (unusual combination)
        if has_cat_blessing and has_doubt:
            return {
                'type': 'special_both',
                'title': 'Откровения?',
                'description': [
                    ...
                    description_4,
                    "Что ты хочешь этим сказать?! Думаешь, что я вру тебе? Да ты хоть знаешь сколько раз я уже пытаюсь поставить тебя на истинный путь?",
                    "Мне столько раз приходилось объяснять тебе одно и то же... Столько раз я помогал тебе проживать одни и те же моменты...",
                    "И вот после всего этого ты говоришь, что не можешь мне доверять?! Какой же ты... Хотя знаешь? Я же твой друг. Я все равно готов тебе помочь, только...",
                    description_5,
                    "Я тебе использовал? Ха, не смеши меня. Зачем мне это слабое тело?",
                    "Вы осознали свою ошибку. Вы больше не будете пытаться начать все заново. Память обо всех допущенных ошибках поможет вам. Вы выбрались из петли..."
                ],
                'stats': {
                    'final_stress': stress,
                    'final_paranoia': paranoia,
                    'voice_relationship': voice_relationship,
                    'special_status': 'both_statuses',
        ...
        # SPECIAL ENDING: Cat blessing only (good route with cats)
        elif has_cat_blessing:
            return {
                'type': 'special_cats',
                'title': 'Что-то незримое',
                'description': [
                    description_1,
                    description_2,
                    description_6,
        ...
        # SPECIAL ENDING: Doubt only (complex route with existential doubt)
        elif has_doubt:
            return {
                'type': 'special_doubt',
                ...
                'description': [
                    description_1,
                    description_2,
                    "Но тут что-то было не так. Вам вспомнился тот диалог с незнакомкой. ",
                    "Все-таки голос вам врал все это время... Или что-то не договаривал. "
                    "Это не так важно. Суть в том, что теперь доверять некому. "
                    "Видимо теперь только один выход...  ",
                ],
                'stats': {
                    'final_stress': stress,
                    'final_paranoia': paranoia,
                    'voice_relationship': voice_relationship,
                    'special_status': 'doubt',
                    'final_score': final_score,
                    'total_tasks_assigned': total_tasks_assigned,
                    'total_tasks_completed': total_tasks_completed,
                    'task_completion_rate': task_completion_rate
    ...
    def draw_header(self):
        ...
        clock_center_x = WINDOW_WIDTH - 110
        clock_center_y = 260
        clock_radius = 70

        draw_analog_clock(self.screen, clock_center_x, clock_center_y, clock_radius, self.game_state.current_time)

        # Можно добавить цифровое время маленьким шрифтом под часами
        if 'small' in FONTS:
            time_str = f"{int(self.game_state.current_time):02d}:{int((self.game_state.current_time % 1) * 60):02d}"
            time_surf = FONTS['small'].render(time_str, True, COLORS['text_dim'])
            self.screen.blit(time_surf, (clock_center_x - time_surf.get_width() // 2, clock_center_y + clock_radius + 8))
        # Mental state bars
    ...
    def draw_game_ending_screen(self):
        ...
        elif ending_type == 'neutral' or ending_type == 'special_cats' or ending_type == "special_doubts":
            bg_color = (30, 30, 40)  # Dark blue-gray
            accent_color = COLORS['accent']
        elif ending_type == 'special_both':
            bg_color = (20, 40, 20)  # Dark green
            accent_color = COLORS['success']
            accent_color = COLORS['success']
        else:  # bad
        ...
        is_final_ending = self.game_state.phase == 'game_ending' or self.game_state.day >= 30

        # Проверяем идеальную концовку: кошачье благословение + сомнения
        has_cat_blessing = (hasattr(self.game_state, 'permanent_statuses') and
                            'cat_blessing' in self.game_state.permanent_statuses)
        has_doubt = (hasattr(self.game_state, 'permanent_statuses') and
                     'doubt' in self.game_state.permanent_statuses)  # Замени 'doubt' на точное имя, если другое

        is_perfect_ending = is_final_ending and has_cat_blessing and has_doubt

        # Restart button
        mouse_pos = pygame.mouse.get_pos()
        mouse_clicked = pygame.mouse.get_pressed()[0]
        if is_perfect_ending:
            button_text = "Разорвать цикл"
            button_width = 340
            button_size = 'medium'
        elif not is_perfect_ending and has_doubt:
            button_text = "Шагнуть вперед"  # Или "Хорошо", как было раньше
            button_width = 340
            button_size = 'medium'
        else:
            button_text = "Начать заново"  # Или "Хорошо", как было раньше
            button_width = 320
            button_size = 'medium'
        restart_btn = Button(WINDOW_WIDTH // 2 - 100, content_rect.bottom - 60, 200, 40,
                             button_text, button_size)
    ...
    def draw_event_popup(self):
        ...
        if event_type == 'notification':
            ...
            ok_btn = Button(510, 450, 120, 40, "Хорошо", 'medium')

            if (ok_btn.update(mouse_pos, mouse_clicked) and self.can_click_button('event_acknowledge')):
                self.register_button_click('event_acknowledge')
                self.process_event(event, "acknowledge")
                self.resume_activity_if_interrupted()
                # Закрываем попап события (защиту от штрафа снимаем позже — в execution фазе)
            ...
            accept_btn = Button(450, 450, 120, 40, "Принять", 'medium')
            decline_btn = Button(650, 450, 120, 40, "Отклонить", 'medium')
            ...
            time_btn = Button(705, 450, 200, 40, time_text, 'tiny')
    ...
    def apply_card_effect(self, card):
        ...
        if effect_type == 'immediate':
            ...
            print(f"🎴 ОБРАБОТКА КАРТЫ СТАТУСА: {card['name']}")
            print(f"   ID: {status_id}, Эффекты: {status_effects}")
    ...
    def draw_voice_message_popup(self):
        ...
        has_cat_blessing = (
                hasattr(self.game_state, 'permanent_statuses') and
                'cat_blessing' in self.game_state.permanent_statuses
        )
        has_doubt = (
                hasattr(self.game_state, 'permanent_statuses') and
                'doubt' in self.game_state.permanent_statuses  # Если статус сомнений называется иначе — замени 'doubt'
        )

        is_perfect_ending = has_cat_blessing and has_doubt
        # Semi-transparent overlay
        overlay = pygame.Surface((WINDOW_WIDTH, WINDOW_HEIGHT))
        overlay.set_alpha(128)
        overlay.fill((0, 0, 0))
        ...
        if (close_btn.update(mouse_pos, mouse_clicked) and
            ...
            if 'performance_score' in voice_message or 'task_statistics' in voice_message:
                ...
                if dream:
                    ...
                    if dream['type'] == 'warning':
                        ...
                        print(f"⚠️ Предупреждающий сон: {dream['id']}")
                        ...
                        print(f"✨ Мотивирующий сон: {dream['id']}")
                else:
                    print("🛌 Снов не приснилось сегодня")
            else:
                print("📢 Это утреннее сообщение от голоса — сон не генерируем")
        else:
            close_btn.update(mouse_pos, False)
